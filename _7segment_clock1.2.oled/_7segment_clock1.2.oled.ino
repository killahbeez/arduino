...
uint8_t loop_data_oled = 0;
uint8_t loop_oled_cnt = 0;

void draw(void) {
  // graphic commands to redraw the complete screen should be placed here

  switch (loop_data_oled) {
    case 0:
      ...
      if (temperature[1]) {
        ...
        u8g.drawStr( 25, 60, bufferTemp);
        u8g.drawDisc( 115, 15, 7);
      }
      break;

    case 1:
      u8g.setFont(u8g_font_fub20r);
      u8g.drawStr( 0, 34, clock.dateFormat("l", dt));
      u8g.drawStr( 30, 64, clock.dateFormat("d/m", dt));
      break;
...
void loop() {
  ...
  checkSwitchOnOff();
  if (isOn) {
    if (digits_refresh && !button_0_cycle) {
      u8g.firstPage();
      do {
        draw();
      } while ( u8g.nextPage() );
    ...
    if (one_second && !button_0_cycle) {

      if (loop_oled_cnt % 5 == 0) {
        loop_data_oled = ++loop_data_oled % 2;
      }
