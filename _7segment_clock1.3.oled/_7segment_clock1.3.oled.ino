...
void draw(void) {
  ...
  if (!stop_anim) {
    ...
    if (current_millis - oled_millis > 10) {
  ...
  for (int16_t i = 0; i < countArr(stop_slide); i++) {
    if (pixel == stop_slide[i]) {
      if (!stop_anim) {
        stop_millis = current_millis;
      }
      stop_anim = 1;
      if (current_millis - stop_millis > 4000) {
        stop_anim = 0;
      }
      break;
    }
  ...
  u8g.drawStr( pixel, 60, bufferTemp);
  u8g.drawDisc( pixel + 90, 15, 7);
  u8g.setFont(u8g_font_fub20r);
  u8g.drawStr( pixel + 113, 34, clock.dateFormat("l", dt));
  u8g.drawStr( pixel + 143, 64, clock.dateFormat("d/m", dt));
