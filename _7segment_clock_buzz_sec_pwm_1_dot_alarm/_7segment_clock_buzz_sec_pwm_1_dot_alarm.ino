...
void initialize() {
  ...
  if (EEPROM.read(EEP_ADDR.logo) != 255) {
    EEP_VARS.logo = EEPROM.read(EEP_ADDR.logo);
  }
  else {
    EEP_VARS.logo = 0; // default is logo ADI
  }
...
void loop() {
  ...
  //CHANGE TO DATE
  if (!in_setting.clock && !in_setting.date && !in_setting.alarm && display_mode != DATE && display_mode != LOGO && (button_Date.isPressed() || loop_data_show == 1)) {
  ...
  //CHANGE TO TEMP
  if (!in_setting.clock && !in_setting.date && !in_setting.alarm && display_mode != TEMP && display_mode != LOGO && (button_Temp.isPressed() || loop_data_show == 2)) {
  ...
  if (!in_setting.clock && !in_setting.date && !in_setting.alarm) {
    //CHANGE LOGO
    if (button_Temp.isReleased() == 0 && button_Date.isReleased() == 0) {
      ...
      EEP_VARS.logo = ++EEP_VARS.logo % 2;

      EEPROM.write(EEP_ADDR.logo, EEP_VARS.logo);
      EEP_VARS.logo = EEPROM.read(EEP_ADDR.logo);
      ...
      switch (EEP_VARS.logo) {
        case 1:
          Serial.println("Set logo Alssa");
          break;
        case 0:
          Serial.println("Set logo Adi");
          break;
        default:
          Serial.println("Set logo Adi");
      }
      ...
      TCCR2A = TCCR2A & 0b00111111; //led SEC OFF
      TCCR2A = ( TCCR2A & 0b11001111 ) | 0b00100000; //enable buzzer
      OCR2B = 100;
      set_logo.is_changed = 1;
      set_logo.chg_time = millis();
    }

    if ( (millis() - set_logo.chg_time) > 10 && set_logo.is_changed == 1) {
      set_logo.is_changed = 0;
      TCCR2A = TCCR2A & 0b11001111; //disable buzzer
    }

    //CHANGE BUZZING ON SEC
    if (button_Temp.isReleased() == 0 && button_Clock.isReleased() == 0) {
      button_Temp.released = -1;
      button_Clock.released = -1;
      button_Temp.pressed = 0;
      button_Clock.pressed = 0;
...
void setSR_Logo(uint8_t pos, uint8_t* logo, uint8_t size) {
  resetNumbers();
  for (uint8_t i = 0; i < size; i++) {
    if ( (pos - i) >= 0 && (pos - i) <= 6) shiftReg[pos - i] = logo[i];
  }
}

void showLogo(uint8_t* logo, uint8_t size) {
  uint8_t limit_scroll = size + 6;
  uint8_t stop_scroll = limit_scroll + 4 + (size / 2);

  if (isr_ticker.ms_100)  {
    scroll_text = 1;
    if (scroll_cnt <= stop_scroll) {
      scroll_cnt++;
    }
  ...
  if (pos_logo >= 0) {
    setSR_Logo(pos_logo, logo, size);
    if (pos_logo - crtDigit >= 0) {
      writeDigits(shiftReg[pos_logo - crtDigit], pos_logo - crtDigit);
    }
    crtDigit = (++crtDigit) % size;
  }
}

void setSR_Loop(uint8_t pos, uint8_t* loop, uint8_t size) {
  resetNumbers();
  if (pos <= 6) {
    for (uint8_t i = 0; i < size; i++) {
      if ( (pos - i) >= 0) shiftReg[pos - i] = loop[i];
    }
  }
}
...
void showLoop(boolean on = 1) {
  ...
  if (pos_logo >= 0) {
    if (on) {
      setSR_Loop(pos_logo, loop_on, sizeof(loop_on));
      crtDigit = (++crtDigit) % countArr(loop_on);
    }
    else {
      setSR_Loop(pos_logo, loop_off, sizeof(loop_off));
...
void digitsRefresh() {
  ...
  if (display_mode == SET_ALARM) {
    ...
    if (in_setting.alarm) {
      ...
      setSR_Snapshot(snapshot);
      number_digits = (freeze_alarm.hour < 10) ? 5 : 6;
    }
