...
uint8_t calup_oled = 0;
void draw(void) {
  static int16_t pixel = 128;
  static char bufferTemp[6];
  ...
  switch (calup_oled) {
    ...
    case 1:
      ...
      u8g.drawStr( pixel, 34, clock.dateFormat("l", DS3231_date));
      u8g.drawStr( pixel + 30, 64, clock.dateFormat("d/m", DS3231_date));
      break;
    case 2:
      pixel = 0;
...
void loop() {
  ...
  if (isr_ticker.sec_5) {
    u8g.firstPage();
    calup_oled = ++calup_oled % 3;
    isr_ticker.sec_5 = false;
    do{
       draw();
    }while(u8g.nextPage());
  }
...
ISR(TIMER1_OVF_vect) {
  ...
  // 5 second ticker
  if (cnt % (freq_multiplex * 5) == 0) {
    isr_ticker.sec_5 = true;
  }
