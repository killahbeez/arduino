...
void loop() {
  if (isOn) {
    ...
    if (button_Clock.isPressed() && clock_set) {
      ...
      snapshot_stage = set_clock_cycle;
    ...
    //SHOW Clock
    if (!set_clock_cycle && !set_date_cycle && display_mode != CLOCK && (button_Clock.isPressed() || loop_data_show == 0)) {
      display_mode = CLOCK;
      show_date = 0;
      show_temperature = 0;
      loop_data_show = 3;
    ...
    //SHOW Temperature
    if (!set_clock_cycle && !set_date_cycle && (button_Temp.isPressed() || loop_data_show == 1) ) {
      show_temperature = !show_temperature;
      show_temperature = 1;
    ...
    //SHOW Date
    if (!set_clock_cycle && !set_date_cycle && (button_Date.isPressed() || loop_data_show == 2) ) {
      show_date = !show_date;
      show_date = 1;
...
void settingClock(uint8_t snapshot[], freeze_clock_t &freeze) {
  if (button_Temp.isReleased() == 0) { // if UP button is pressed
    ...
    if (set_clock_cycle == 1 && diffMillis > speed_scroll_set) { // if UP on HH
    ...
    if (set_clock_cycle == 2 && diffMillis > speed_scroll_set) { // if UP on MM
    ...
    if (set_clock_cycle == 3 && diffMillis > speed_scroll_set) { // if UP on SS
  ...
  if (button_Date.isReleased() == 0) { // if DOWN button is pressed
    ...
    if (set_clock_cycle == 1 && diffMillis > speed_scroll_set) { // if DOWN on HH
    ...
    if (set_clock_cycle == 2 && diffMillis > speed_scroll_set) { // if DOWN on MM
    ...
    if (set_clock_cycle == 3 && diffMillis > speed_scroll_set) { // if DOWN on SS
