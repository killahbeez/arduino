...
volatile boolean tens_seconds_flag = 0;
...
uint8_t set_clock_cycle = 0;
boolean clock_set = 0;

uint8_t set_date_cycle = 0;
boolean date_set = 0;

uint8_t set_alarm_cycle = 0;
boolean alarm_set = 0;
boolean show_set_alarm = 0;
char get_alarm[6];
uint8_t is_armed = 0;
...
void loop() {
  ...
  if (isOn) {
    ...
    if (!buzzing && tens_seconds_flag && is_armed && clock.isAlarm1(false)) {
    ...
    //SETTING CLOCK
    //if the button 1 is pressed for more than 2 sec enter the setting clock
    //cycle throw setting pairs SS MM HH
    //on the fourth press release the setting stage
    if (button_Clock.isPressedFor(2000) && display_mode == CLOCK && !clock_set) {
      cli();
      clock_set = 1;
      set_clock_cycle = 1;
    ...
    if (button_Clock.isPressed() && clock_set) {
      if (set_clock_cycle == 3) {
        tens_seconds = 0;
        clock_set = 0;
      ...
      set_clock_cycle = ++set_clock_cycle % 4;
      button_Clock.pressed = 0;
      button_Clock.released = -1;
    }

    if (set_clock_cycle) {
      settingClock(snapshot, freeze_clock, set_clock_cycle);
    }
    //END SETTING CLOCK

    //SETTING DATE
    //if the date is pressed for more than 2 sec enter the setting date
    //cycle throw setting pairs DD MM YY
    //on the fourth press release the setting stage
    if (button_Date.isPressedFor(2000) && display_mode == DATE && !date_set) {
      cli();
      date_set = 1;
      set_date_cycle = 1;

      getSnapshot(snapshot);
      freeze_date.day = dt.day;
      freeze_date.month = dt.month;
      freeze_date.year = dt.year - 2000;
      PORTB &= ~(1 << PB3);
    ...
    //SETTING ALARM
    //if the alarm is pressed enter the setting alarm mode
    //cycle throw setting pairs HH MM SS
    //on the fourth press release the setting stage
    if (show_set_alarm && !set_clock_cycle && !set_date_cycle && !alarm_set) {
      alarm = clock.getAlarm1();
      sprintf(get_alarm, "%02d%02d%02d", alarm.hour, alarm.minute, alarm.second);

      display_mode = SET_ALARM;

      setAlarmDigits(get_alarm);
      getSnapshot(snapshot);

      freeze_alarm.hour = alarm.hour;
      freeze_alarm.minute = alarm.minute;
      freeze_alarm.second = alarm.second;
      ...
      button_SetAlarm.released = -1;
    }
    if (button_Clock.isPressed() && alarm_set) {
      if (set_alarm_cycle == 3) {
        alarm_set = 0;
        show_set_alarm = 0;
        setAlarm();
        //seteaza alarma
      ...
      set_alarm_cycle = ++set_alarm_cycle % 4;
      button_Clock.pressed = 0;
      button_Clock.released = -1;
    }

    if (set_alarm_cycle) {
      settingClock(snapshot, freeze_alarm, set_alarm_cycle);
    }
    //END SETTING ALARM

    //SHOW Clock
    if (!set_clock_cycle && !set_date_cycle && !set_alarm_cycle && display_mode != CLOCK && (button_Clock.isPressed() || loop_data_show == 0)) {
      display_mode = CLOCK;
      show_date = 0;
      show_temperature = 0;
      loop_data_show = 3;
    ...
    //SHOW Temperature
    if (!set_clock_cycle && !set_date_cycle && !set_alarm_cycle && (button_Temp.isPressed() || loop_data_show == 1) ) {
      show_temperature = !show_temperature;
      show_temperature = 1;
    ...
    //SHOW Date
    if (!set_clock_cycle && !set_date_cycle && !set_alarm_cycle && (button_Date.isPressed() || loop_data_show == 2) ) {
      show_date = !show_date;
      show_date = 1;
    ...
    //SHOW Set ALARM
    if (!set_clock_cycle && !set_date_cycle && button_SetAlarm.isPressed() ) {
      show_set_alarm = !show_set_alarm;
      if (!show_set_alarm) {
        set_alarm_cycle = 0;
        alarm_set = 0;
        setAlarm();
    ...
    //ARM ALARM
    if (!set_clock_cycle && !set_date_cycle && button_ArmAlarm.isPressed() ) {
      if (!clock.isArmed1()) {
    ...
    if (toggle_leds_second && !set_clock_cycle && display_mode == CLOCK) {
      PORTB ^= (1 << PB3);
...
void settingClock(uint8_t snapshot[], freeze_clock_t &freeze, uint8_t cycle) {
  if (button_Date.isReleased() == 0) { // if UP button is pressed
    if (button_Date.first_press) {
      speed_scroll_set = 200;
    }
    if (cycle == 1 && diffMillis > speed_scroll_set) { // if UP on HH
    ...
    if (cycle == 2 && diffMillis > speed_scroll_set) { // if UP on MM
    ...
    if (cycle == 3 && diffMillis > speed_scroll_set) { // if UP on SS
  ...
  if (button_Temp.isReleased() == 0) { // if DOWN button is pressed
    ...
    if (cycle == 1 && diffMillis > speed_scroll_set) { // if DOWN on HH
    ...
    if (cycle == 2 && diffMillis > speed_scroll_set) { // if DOWN on MM
    ...
    if (cycle == 3 && diffMillis > speed_scroll_set) { // if DOWN on SS
...
void settingDate(uint8_t snapshot[], freeze_date_t &freeze) {
  if (button_Date.isReleased() == 0) { // if UP button is pressed
    ...
    if (set_date_cycle == 1 && diffMillis > speed_scroll_set) { // if UP on DD
    ...
    if (set_date_cycle == 2 && diffMillis > speed_scroll_set) { // if UP on MM
    ...
    if (set_date_cycle == 3 && diffMillis > speed_scroll_set) { // if UP on YY
  ...
  if (button_Temp.isReleased() == 0) { // if DOWN button is pressed
    ...
    if (set_date_cycle == 1 && diffMillis > speed_scroll_set) { // if DOWN on DD
      freeze.day = --freeze.day;
      if (freeze.day < 1) {
        freeze.day = 31;
      }
    ...
    if (set_date_cycle == 2 && diffMillis > speed_scroll_set) { // if DOWN on MM
      freeze.month = --freeze.month;
      if (freeze.month < 1) {
        freeze.month = 12;
      }
    ...
    if (set_date_cycle == 3 && diffMillis > speed_scroll_set) { // if DOWN on YY
...
void setAlarmDigits(char number[]) {
  ATOMIC_BLOCK(ATOMIC_RESTORESTATE) {
    uint8_t cnt = 1;
    resetNumbers();
    for (int8_t i = 5; i >= 0; i--) {
...
void digitsRefresh() {
  ...
  if (display_mode == LOOP_ON || display_mode == LOOP_OFF) {
    if (scroll_cnt <= 6) {
      ...
      cnt_refresh++;
      if (cnt_refresh > freq_multiplex * 2) { //after 2 seconds go the temp display
        scroll_cnt = 0;
        display_mode = CLOCK;
        cnt_refresh = 0;
      }
  ...
  if (display_mode == CLOCK) {
    ...
    if (set_clock_cycle) { // HH

      if (set_clock_cycle == 1) {
        snapshot[5] = snapshot[5] | 1; // show setting state on DOT
      }
      if (set_clock_cycle == 2) {
  ...
  if (display_mode == SET_ALARM) {
    ...
    if (set_alarm_cycle) { // HH
      if (set_alarm_cycle == 1) {
        snapshot[5] = snapshot[5] | 1; // show setting state on DOT
      }
      if (set_alarm_cycle == 2) {
        snapshot[5] &= ~(1 << 0); //reset the old dot
        snapshot[3] = snapshot[3] | 1; // show setting state on DOT
      }
      if (set_alarm_cycle == 3) {
