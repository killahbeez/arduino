...
char* DS3231::dateFormat(const char* dateFormat, RTCTime dt)
{
    char buffer[255];
...
char* DS3231::dateFormat(const char* dateFormat, RTCDate dt)
{
    char buffer[255];
...
void DS3231::getTime(RTCTime &time) {
	int values[3];
    ...
    time.second = values[0];
    time.minute = values[1];
    time.hour = values[2];
}


void DS3231::getDate(RTCDate &date) {
	int values[4];
    ...
    date.dayOfWeek = values[0];
    date.day = values[1];
    date.month = values[2];
    date.year = values[3] + 2000;
}
