...
void loop() {
  ...
  if(!buzzing){
    if (display_mode == CLOCK) {
      if (DS3231_time.tens == 0 && !beep_sec) {
        TCCR2A = ( TCCR2A & 0b11001111 ) | 0b00100000; //enable PWM for PD3
        beep_sec = true;
      }
      if (beep_sec) {
        secBuzzer();
        if (DS3231_time.tens == 1) {
          beep_sec = false;
          TCCR2A = TCCR2A & 0b11001111; //disable PWM for PD3
        }
      }
...
void digitsRefresh() {
  ...
  if (display_mode == CLOCK) {
    ...
    if (!in_setting.clock) {
      ...
      if (isr_ticker.ms_500_leds_second) {
        ...
        else { // on 1/2 second turn off leds
          PORTB &= ~(1 << PB3);
        }
  ...
  if (loop_data && display_mode != LOGO && isr_ticker.sec_1_loop) {
    //after 30 sec loop data for 1 sec
    if (loop_data_cnt == 30) {
      loop_data_show = 1;
    }
    if (loop_data_cnt == 31) {
...
void secBuzzer() {
  buzzer_sec_vars.cnt = 40;
  //if (buzzer_sec_vars.tick == 1) {
    OCR2B = buzzer_sec_vars.cnt;

    buzzer_sec_vars.tick = 0;
  //}
}
