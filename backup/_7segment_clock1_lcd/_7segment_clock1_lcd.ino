...
void drawLCD() {
  static boolean show_degree = 1;
  static int8_t last_temperature;
  static int8_t current_temperature;
  ...
  current_temperature = (int8_t) clock.readTemperature();
...
void loop() {
  ...
  if (isOn) {
    ...
    if (digits_refresh) {
      ...
      if (display_mode == 3) {
        PORTB |= (1 << PB3);
        char* data = clock.dateFormat("dmy", dt);
