...
void loop() {
  ...
  IRdim(0x490, 0xC90, DIM_LIGHT, pushed_lamp_1);
  ...
  IRdim(0x90, 0x890, SPEED_FAN, pushed_lamp_2);
...
void IRdim(uint32_t up, uint32_t down, volatile uint16_t &dim, uint32_t &start_press) {
  ...
  if (ircomm == up || ircomm == down) {

    if (millis() - last_mil > timelapsed) {
      if (ircomm == up && dim <= 253) {
        dim+=2;
        start_press++;
      }
      if (ircomm == down && dim >= 2) {
        dim-=2;
        start_press++;
      }
      last_mil = millis();
  ...
  else if ( (millis() - last_mil) > (timelapsed + 100)) {
    timelapsed = 200;
    start_press = 0;
  }
}
