...
void loop() {
  ...
  if (lcd_on) {
    if (buttonPressed(1) || ircomm == 0xFFD02F || ircomm == 0xB38) { //remote button AUX
      ...
      updateLF[0] = 1;
      updateLF[1] = 1;
      displayFan(SPEED_FAN);
      displayLamp(DIM_LIGHT);
    }
    if (refreshLCD) {
...
uint32_t diffMillis(uint32_t from){
  return (millis()-from); 
}

boolean fadeLampOn = 0;
boolean fadeLampOff = 0;
uint32_t startFadeMillis = 0;
uint8_t fadeCntLamp = 0;

void fadeLamp(boolean dir){
      if(diffMillis(startFadeMillis) > 2){
        
        if(dir == 1 && fadeLampOn){ //fade up
          if(++fadeCntLamp <= DIM_LIGHT){
            OCR0A = fadeCntLamp;
            updateLF[0] = 1;
            displayLamp(fadeCntLamp);
          }
          else{
            fadeLampOn = 0;
          }
        }
        else if(dir == 0 && fadeLampOff){ // fade down
          if(--fadeCntLamp >= 0){
            OCR0A = fadeCntLamp;
            updateLF[0] = 1;
            displayLamp(fadeCntLamp);
...
void switchLamp() {
  if (buttonPressed(2) || ircomm == 0xFFF00F || ircomm == 0x738) { //remote button CH
    ...
    if (lamp_on) {
      DDRD |= (1 << PD6);
      startFadeMillis = millis();
      fadeCntLamp = 0; // reset counter to 0
      fadeLampOn = 1;
                //OCR0A = DIM_LIGHT;
                //updateLF[0] = 1;
    }
      ...
      startFadeMillis = millis();
      fadeCntLamp = DIM_LIGHT; // reset counter to dim_light
      fadeLampOff = 1;
                //DDRD &= ~(1 << PD6);
                //PORTD &= ~(1 << PD6);
                //clearDisplayLamp();
...
void displayLamp(uint8_t SPEED) {
  char temp[10];
  ...
  if (lamp_on && updateLF[0]) {
    ...
    proc_lamp = (uint8_t) ((SPEED * 100) / 255);
