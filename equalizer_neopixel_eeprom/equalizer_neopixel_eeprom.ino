...
uint8_t EEP_ADDR_MODE = 0;
uint8_t EEP_ADDR_COLORS = 1;
uint8_t EEP_ADDR_DIM = 2;

boolean writeEEPROM = 0;
...
void setup() {
  ...
  if (EEPROM.read(EEP_ADDR_MODE) != 255 && EEPROM.read(EEP_ADDR_COLORS) != 255 && EEPROM.read(EEP_ADDR_DIM) != 255) {
    modeType = EEPROM.read(EEP_ADDR_MODE);
    cntColors = EEPROM.read(EEP_ADDR_COLORS);
    brightness = EEPROM.read(EEP_ADDR_DIM);
  }
...
void loop() {
  ...
  if (writeEEPROM && (millis() - EEP_millis) >= 2000) {
    EEPROM.write(EEP_ADDR_MODE, modeType);
    EEPROM.write(EEP_ADDR_COLORS, cntColors);
    EEPROM.write(EEP_ADDR_DIM, brightness);

    writeEEPROM = 0;

    if (debug) {
      Serial.print("EEPROM MODE write: ");
      Serial.println(EEPROM.read(EEP_ADDR_MODE));
      Serial.print("EEPROM COLORS write: ");
      Serial.println(EEPROM.read(EEP_ADDR_COLORS));
      Serial.print("EEPROM DIM write: ");
      Serial.println(EEPROM.read(EEP_ADDR_DIM));
    }
  }
