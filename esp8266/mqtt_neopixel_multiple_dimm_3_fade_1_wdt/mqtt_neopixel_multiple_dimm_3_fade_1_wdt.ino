...
void sliderNeopixel() {
  uint32_t color;
  char *culori[] = {"ff0000","00ff00","0000ff"};
  static uint8_t i = 0;
  hex_colors RGB_colors = getRGB(culori[i%3]);
  i++;

  for (uint8_t i = 0; i < 3; i++) {
    pixels[i].clear();
    pixels[i].setBrightness(100);
    color  = pixels[i].Color(RGB_colors.R, RGB_colors.G, RGB_colors.B);

    for (int8_t cnt = pixels[i].numPixels() - 1; cnt >= 0; cnt--) {
      pixels[i].setPixelColor(cnt, color);
      pixels[i].show();
      delay(5);
    }
  }
  ...
  for (int8_t i = 2; i >=0 ; i--) {
    RGB_colors = getRGB("000000");
    color  = pixels[i].Color(RGB_colors.R, RGB_colors.G, RGB_colors.B);
    for (int8_t cnt = 0; cnt < pixels[i].numPixels(); cnt++) {
      pixels[i].setPixelColor(cnt, color);
      pixels[i].show();
      delay(5);
    }
