...
void setup() {
  ...
  //TCCR0B = (1 << CS01) | (1 << CS00); //clk/64 prescalling freq = 976 Hz
...
void loop() {
  ...
  last_DIM_LIGHT = DIM_LIGHT;
  IRdim(0xFD20DF, 0xFD10EF, DIM_LIGHT, pushed_lamp_1);
  if (last_DIM_LIGHT != DIM_LIGHT) {
    updateLF[0] = 1;
    displayLamp();
  }

  last_SPEED_FAN = SPEED_FAN;
  IRdim(0xFDA05F, 0xFD906F, SPEED_FAN, pushed_lamp_2);
  if (last_SPEED_FAN != SPEED_FAN) {
    updateLF[1] = 1;
    displayFan();
  }

  if (pushed_lamp_1 || pushed_lamp_2) {
    TIMSK1 &= ~(1 << TOIE1);
  }
  else {
    TIMSK1 = (1 << TOIE1);
  ...
  switchFan();
  if (lcd_on) {
    if (buttonPressed(1) || ircomm == 0xFD30CF) { //remote button AUX
      lcd.clear();
      lcd_cycle++;
      if (lcd_cycle == 4) {
        lcd_cycle = 0;
...
void switchLamp() {
  if (buttonPressed(2) || ircomm == 0xFD00FF) { //remote button CH
    ++lamp_on %= 2;
    if (lamp_on) {
      DDRD |= (1 << PD6);
      OCR0A = DIM_LIGHT;
...
void switchFan() {
  if (buttonPressed(3) || ircomm == 0xFD807F) { //remote button DVD
...
void displayFan() {
  ...
  if (fan_on && updateLF[1]) {
    ...
    lampChar = (uint8_t)((proc_fan / 25) + 2);
    if (lampChar > 5) lampChar = 5;
...
void switchLCD() {
  if (buttonPressed(0) || ircomm == 0xFDA857) { //remote button 3D
    ++lcd_on %= 2;
...
void IRdim(uint32_t up, uint32_t down, volatile uint16_t &dim, uint32_t &start_press) {
  ...
  static uint16_t timelapsed = 300;
