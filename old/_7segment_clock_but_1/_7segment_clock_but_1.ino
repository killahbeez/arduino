...
void loop() {
  ...
  if (buttonPressed(0)) {
    if (button_0_cycle == 3) {
      clock.setDateTime(dt.year, dt.month, dt.day, constrain(freeze_hour, 0, 59), constrain(freeze_minute, 0, 59), constrain(freeze_second, 0, 59)); //set CLOCK for SS change
    }
  ...
  if (toggle_leds_second && !button_0_cycle) {
    PORTB ^= (1 << PB3);
