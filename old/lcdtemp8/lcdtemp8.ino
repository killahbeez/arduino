...
void fadeLamp(uint16_t SPEED_DIM,boolean dir){
  if(dir){
    for(uint16_t i=0;i<SPEED_DIM;i++){
      OCR0A = i;
      updateLF[0] = 1;
      displayLamp(i);
      //delayNew(1);
    ...
    lamp_on = 1;
    for(uint16_t i=SPEED_DIM;i>0;i--){
      OCR0A = i;
      updateLF[0] = 1;
      displayLamp(i);
      //delayNew(1);
...
void switchLamp(uint16_t SPEED_DIM) {
  if (buttonPressed(2) || ircomm == 0xFFF00F || ircomm == 0x738) { //remote button CH
    ++lamp_on %= 2;
    if (lamp_on) {
      DDRD |= (1 << PD6);
      fadeLamp(SPEED_DIM,1);
    }
    else {
      fadeLamp(SPEED_DIM,0);
      DDRD &= ~(1 << PD6);
      PORTD &= ~(1 << PD6);
      clearDisplayLamp();
    }
  ...
  if (lamp_on) {
    ...
    if (updateLF[0]) {
      displayLamp(SPEED_DIM);
    }
  }
}


void switchFan(uint16_t SPEED_DIM) {
  ...
  if (fan_on) {
    ...
    if (updateLF[1]) {
      displayFan(SPEED_DIM);
    }
  }

}

void displayLamp(uint16_t SPEED_DIM) {
  char temp[10];
  ...
  if (lamp_on && updateLF[0]) {
    ...
    proc_lamp = (uint8_t) ((SPEED_DIM * 100) / 255);
...
void displayFan(uint16_t SPEED_DIM) {
  char temp[10];
