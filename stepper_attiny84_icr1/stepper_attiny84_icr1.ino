...
void setup() {
  ...
  //setting timer 1
  TCCR1A = (1 << COM1A1) | (1 << WGM11); // Fast PWM ICR1
  TCCR1B = (1 << WGM13) | (1 << WGM12) | (1 << CS12) ; // FCPU/256 prescaler
  TIMSK1 = (1 << OCIE1A) | (1 << TOIE1); //Timer/Counter1, Output Compare A Match Interrupt Enable + Overflow Interrupt Enable
...
ISR(ADC_vect) {
  ...
  if (cnt >= 100) {
    currentPotValue = constrain(map((sum / 100), 2, 250, 2, 300), 2, 300);
...
void set_step_delay(float delayed, boolean set_dir) //milliseconds
{
  ...
  ICR1_temp = F_CPU / delay_long;
  ICR1 = (ICR1_temp > 65535 ? 65535 : ICR1_temp);

  if (delayed >= 15) {
    if (delayed >= 50) {
      OCR1A = (uint16_t) ICR1 / 10; // 1/10th duty cycle
    }
    else {
      OCR1A = (uint16_t) ICR1 / 3; // 1/10th duty cycle
    }
  }
  else {
    OCR1A = (uint16_t) ICR1; // 100% duty cycle
  }
  sei();
}
