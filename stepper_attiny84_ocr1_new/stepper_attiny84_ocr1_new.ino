...
void setup() {
  ...
  //SET INPUT_PULLUP RESISTOR FOR BUTTONS INPUTS + STOP LIMIT
  PORTA |= (1 << MANUAL_FWD_PIN);
  PORTB |= (1 << MANUAL_BCK_PIN);
  PORTB |= (1 << STOP_MIN);
  PORTB |= (1 << STOP_MAX);
...
ISR(ADC_vect) {
  ...
  if (cnt >= 100) {
    ...
    currentPotValue = map((sum / 100), 0, 250, 2, 100);

    if ((currentPotValue < lastPotValue - 3 || currentPotValue > lastPotValue + 3) && currentPotValue != lastPotValue) {
...
void set_step_delay(float delayed, boolean set_dir) //milliseconds
{
  ...
  if (delayed >= 5) {
    if (delayed >= 10) {
      ...
      OCR1B = (uint16_t) OCR1A / 3; // 1/3th duty cycle
    }
  }
  else {
